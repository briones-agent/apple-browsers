...
final class AIChatOmnibarController {
    ...
    func openNewVoiceChat() {
        // No-op in global mode where there's no host tab collection.
    ...
    private func prewarmAttachedTab(id: String) {
        // No origin in global mode — there are no tab attachments to prewarm anyway.
        guard let tabCollectionViewModel else { return }
    ...
    func openTabsForOmnibarPicker() -> [AIChatTabAttachment] {
        // Global mode (floating panel) has no host tab collection — the picker UI isn't reachable
        // there anyway, but defend the call site cleanly.
        guard let tabCollectionViewModel else { return [] }
    ...
    func submit() {
        ...
        // `tabCollectionViewModel` is optional after M4 (global mode passes nil).
        ...
        Task { @MainActor in
            ...
            if isGlobalMode {
                // Global mode (floating panel) has no tab picker / file picker, so `pageContextPayload`
                // and `files` are nil here. Match the address-bar branch's call shape so the global
                // submit hand-off preserves the same prompt envelope.
    ...
    private func extractPageContextsForOmnibarSubmit(
        ...
        // Global mode (floating panel) doesn't expose the tab picker, so tab attachments only ever
        // populate in address-bar mode where `tabCollectionViewModel` is non-nil. Guard defensively
        // and bail with an empty payload if the field is nil — `origin` is required non-optional.
        guard let origin = tabCollectionViewModel else { return nil }
